...
- **The Library/Binary Crate Pattern:** The repository uses a library crate
  (`src/lib.rs`) and a thin binary crate (`src/main.rs`).15 Core logic lives in
  the library so unit and integration tests can exercise it directly, while the
  binary only parses command-line arguments and invokes the library. This
  structure complements the black-box E2E suite and keeps the codebase modular.
...
## Works cited
 ...
 1. Complete Guide To Testing Code In Rust | Zero To Mastery, accessed on July
    20, 2025,
    <https://zerotomastery.io/blog/complete-guide-to-testing-code-in-rust/>

 1. Ultimate Guide to Testing and Debugging Rust Code | 2024 - Rapid
