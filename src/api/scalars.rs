...
//! example `DateTime`, `URI`, and `BigInt`) that have no built-in mapping, so a
...
// `DateTime` is consumed by the reviews operation (`submittedAt`), `URI` by the
// review-thread operations (the comment `url` field), and `BigInt` by resolve
// (`fullDatabaseId`).
