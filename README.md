...
If you pass just a number, `vk` looks for the repository URL in `.git/FETCH_HEAD` first. If that fails it checks the `VK_REPO` environment variable. If neither are present `vk` will error.
